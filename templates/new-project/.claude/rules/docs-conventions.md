...
> **来源**:[#22](https://github.com/m42981454-tech/doc/issues/22)(2026-05-19 引入 path-scoped CLAUDE.md)→ [#23](https://github.com/m42981454-tech/doc/issues/23)(2026-05-20 迁到 `.claude/rules/` 集中管理 / 方案 B)
...
### 1.1 月份归档(2026-05-19 起,refs #22)

**新建近月 doc 入 `docs/YYYY-MM/<file>.md`**(月份目录),不再裸放 docs/ 顶层。

| 类型 | 命名 | 示例 |
|---|---|---|
| 决策(architecture / tech / process 选型) | `docs/YYYY-MM/YYYY-MM-DD_decision_<topic_snake>.md` | `docs/2026-05/2026-05-19_decision_agents-vault-rollout.md` |
| 分析 / audit / spike | `docs/YYYY-MM/YYYY-MM-DD_<topic_snake>.md` | `docs/2026-05/2026-05-19_agent-token-economics.md` |
| 设计提案(未实施) | `docs/YYYY-MM/YYYY-MM-DD_proposal_<topic>.md` | (历史)`archive/2026-05-15_PROPOSAL_api_key_invocation_refactor.md` |
| 接力卡(handoff) | `docs/YYYY-MM/YYYY-MM-DD_handoff_<topic>.md` 或 sprint 专辑内 `99-next-session-handoff.md` | — |
| **例外保留顶层**(无日期 / 固化) | `<scope>_runbook.md` / `<topic>-proposal.md`(历史) | `admin_runbook.md` / `admin-auth-proposal.md` |

**铁律**:
- 新建 doc **必含 `YYYY-MM-DD_` 日期前缀**(除固化 runbook)
- 新建 doc **必入对应月份目录 `docs/YYYY-MM/`**(2026-05-19 起新规约)
- 同一天多个 doc 用 `_<topic>` 区分
- 月份目录用 4 位年 + 2 位月,中间 `-`:`2026-05/` 不是 `202605/`(对齐 doc 日期 prefix 格式)

**迁移现状**(2026-05-19 已迁):
- `docs/2026-05/` 含 2026-05 内 4 个顶层 doc(`agent-token-economics` / `decision_agents-vault-rollout` / `decision_extension-architecture` / `my-agents-optimization`)
- **不动**已有 sprint 专辑(`20260516/` `20260517-confirm/` `20260518-confirm/` 等)— 它们是 numbered sprint set,不是单 doc(避免破坏既有 cross-ref)
- **不动** `docs/archive/`(已归档,保留原 path 不破坏 git history)
- **不动** `docs/2026-05-16_thinking/`(已是目录形态 thinking 集)
...
| 示例 | `docs/20260518-confirm/` | `00-design-and-execution-plan.md` / `01-unused-code-cleanup-report.md` / ... |
...
| 目录 | 内容 | 何时进 |
|---|---|---|
| **`docs/YYYY-MM/`**(月份目录,**新建首选**) | 该月单 doc(decision / analysis / proposal / handoff)| 2026-05-19 起新建 |
| `docs/`(顶层) | 仅例外:无日期固化 runbook + 历史无日期 proposal | 不新写 |
| `docs/archive/` | 历史 design / decision / proposal / ph<N> sprint 报告 | doc 超 30 day 不被引用 → PM 在下次 chore 移入 |
| `docs/brainstorm/` | 启发式 brainstorming notes(R1-R20 系列等) | brainstorm 阶段产出 |
...
- **新建单 doc 必入 `docs/YYYY-MM/`**(自 2026-05-19);sprint 专辑仍按 `YYYYMMDD/` 整体形态(向下兼容,不动既有)
...
> **Refs**: [#N](https://github.com/m42981454-tech/doc/issues/N)  (issue 必带 — 若无 issue,留 "n/a")
...
- **GitHub issue / PR**:`[#N](https://github.com/m42981454-tech/doc/issues/N)`(repo 名必含,跨项目可读)
- **本 repo 内其他 doc**:`[../foo.md](../foo.md)` 或 `path/foo.md`(相对路径)
- **commit hash**:反引号包,可附 link `[abc1234](https://github.com/m42981454-tech/doc/commit/abc1234)`
...
| **doc 内容明显过时**(版本号 / commit hash 引用 commit 已 revert / sprint 状态)| 优先**头部加 deprecated 标注 + link 到替代 doc**;不直接覆盖(保留历史)|
...
- ❌ **不在 docs 下放代码 / 配置 / 二进制**(SQL migration / Python / JSON / 图片大于 100KB 等)— 这些进 `database/migrations/` / `app/` / 项目根
...
新建以下类型 doc 时,先看现有范例 + 模板:

| 类型 | 范例 / 模板 |
|---|---|
| Decision | `docs/2026-05/2026-05-19_decision_agents-vault-rollout.md`(完整范例)/ `archive/2026-05-08_decision_frontend_versions.md`(简短英文风格)|
| Postmortem | `postmortems/README.md` §模板(必照抄结构)|
| Sprint 专辑总纲 | `20260518-confirm/00-design-and-execution-plan.md` |
| Spike / 分析 | `docs/2026-05/2026-05-19_agent-token-economics.md`(token 拆解 + 决策表)|
| Future roadmap | `20260518-future/07-final-future-summary.md`(优先级 + 不做项)|
| Runbook | `admin_runbook.md`(章节化 + smoke test cmd)|
| 全 doc 导航 | `docs/INDEX.md`(分类索引)|

---

## 7. 关联规约(主 CLAUDE.md / 其他 .claude/rules/ 已规定的不重复)

| 不在本文件 | 在哪 |
|---|---|
| Git workflow(子分支 / merge --no-ff / push)| 主 `CLAUDE.md` §7 + `.claude/rules/git-workflow.md` |
| 派 agent / 团队编排 | 主 `CLAUDE.md` §2 + `.claude/rules/team-orchestration.md` |
| Issue-First gate(开 sprint 必先 issue)| `.claude/rules/git-workflow.md §7.9` |
| PROGRESS.md 维护 | `.claude/rules/git-workflow.md §7.7` |
...
## 8. 本文件的加载机制(2026-05-20 — 方案 B 迁移后)
...
- compaction 后,本文件需 Claude 重读 docs 下文件才重 load

**为何迁到 `.claude/rules/` 而非 `docs/CLAUDE.md`(方案 B 优于方案 A)**:
- 集中管理:`.claude/rules/` 已有 4 个 sub-file(`team-orchestration` / `git-workflow` / `project-context` / `behavioral-rules`),本文件加入后形成统一规约目录
- Discovery:grep 找规约时一目了然(对照过去要去 docs/ 翻 CLAUDE.md)
- 跨子目录共享灵活:`paths:` 支持 multi-glob(未来如需扩到 `design/**` `app/docs/**` 等只改 frontmatter)
- 官方推荐:[Claude Code memory docs §Organize rules with .claude/rules/](https://code.claude.com/docs/en/memory#organize-rules-with-claude/rules/)

参 [Claude Code memory docs](https://code.claude.com/docs/en/memory)。

---

> 📌 **Maintainer 注**:本文件由 [#22](https://github.com/m42981454-tech/doc/issues/22) 引入,2026-05-20 [#23](https://github.com/m42981454-tech/doc/issues/23) 从 `docs/CLAUDE.md` 迁到 `.claude/rules/docs-conventions.md`(方案 B)。修改本文件 = CLAUDE.md 改动(per `git-workflow.md §7.9`)→ 必先开 issue + chore 分支。
