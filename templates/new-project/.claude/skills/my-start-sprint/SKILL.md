...
### Step 3 — 开 sprint 子分支(从 feature/dev 起)

子分支命名(per §7.2):
- sprint 任务:`feature/dev.ph<N>` 或 `feature/dev.<container>.<topic>`
- bug 修复:`feature/dev.<container>.fix<M>`
- chore:`feature/dev.<container>.chore.<topic>`

```bash
git checkout -b feature/dev.<topic>
```
...
### Step 7 — sprint 完成 → 走 my-pm-progress-sync

merge 完毕后:
1. `git checkout feature/dev`
2. `git merge --no-ff feature/dev.<topic> -m "Merge feature/dev.<topic> -> feature/dev: <what> (Refs #N)"`
3. `git branch -d feature/dev.<topic>`
4. 调用 [`my-pm-progress-sync`](../my-pm-progress-sync/SKILL.md) 更新 PROGRESS
...
- ❌ 不在主线(feature/dev / main)上 commit(per §7.2 + .githooks/pre-commit 拦)
- ❌ 不直接开 sprint 子分支 skip Issue-First gate(per §7.9)
- ❌ 不在 sprint 分支上 chore.<topic>(必从 feature/dev 起,避免顺手 merge sprint work)
- ❌ 不 push(用户明示前)
