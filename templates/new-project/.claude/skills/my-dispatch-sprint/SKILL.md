...
Agent({
  ...
  prompt: `
    ...
    禁止:扩大重构 / 修改无关文件 / 跳过测试
    起手:必跑 \`git fetch && git checkout feature/dev\` 防 stale base(per 事故 3 教训)
...
Agent({
  ...
  prompt: `
    ...
    Review 标准:
      ...
      3. 测试覆盖:核心 ≥ 90% / 整体 ≥ 80%
      4. 向后兼容:既有 baseline 全绿(主套件 1815/0/44, e2e 58/0/0)
    输出格式:✅ 通过 / ⚠️ 建议 / ❌ 阻断,每项 file:line
  `
})
```

**Tester (API) / Tester (UI) / Security Engineer / Reality Checker / 其他 8 模板** — 详 `.claude/rules/team-orchestration.md §2.7.0-§2.7`。
...
- ❌ 派 future doc agent 漏 sprint 完成清单(per Stage B 教训)
