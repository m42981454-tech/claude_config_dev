...
| 时机 | 强制动作 | 校验 |
|---|---|---|
| 开 sprint 子分支前(`feature/dev.<topic>`) | **必先** `gh issue create` label 含 `bug` / `feature` / `chore` 之一 | sprint 首 commit message 末尾必含 `Refs #N` / `Closes #N` |
| bug 发现时(测试 fail / 用户反馈 / review 阻塞) | 同上 | issue 必须**先**于修复 commit 存在 |
| issue 创建后 | PROGRESS 对应段加 `[#N](issue-url)` backlink | sprint merge 同时 chore 更新 PROGRESS 一并入 |
...
- `<type>`: `bug` / `feature` / `chore` / `security`(label 同名)
- `<模块>`: 例 `chat router` / `admin/auth` / `monitoring` / `docs`
- 例:`[bug] chat router: _stream_with_accounting 多传 quota_service`

### Body 模板

```bash
gh issue create --repo m42981454-tech/doc \
  --title "[<type>] <模块>: <一句话>" \
...
### 关键 label(本 repo 实际 — 实测 2026-05-20 `gh label list`)

| label | 何时用 |
|---|---|
| `bug` | 测试 fail 真根因 / 用户反馈功能问题 / production incident |
| `enhancement` | 新功能 / 增强 / sprint 启动(**注**:GitHub 默认 label;不是 `feature`)|
| `chore` | docs / 重构 / 规则变更 / polish 不改功能 |
| `security` | 鉴权 / 权限 / 加密 / OWASP-relevant 改动 |
| `documentation` | 纯文档(与 chore 区分)|

**铁律**:**新 sprint 前先 `gh label list --repo m42981454-tech/doc` 确认 label 存在**(不存在用 `gh label create` 加,或选语义最近的)。
